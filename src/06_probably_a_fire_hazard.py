...
def first(*instructions):
    ...
    for instruction in instructions:
        match = re.match("(turn on|turn off|toggle) (\d+),(\d+) through (\d+),(\d+)", instruction)
        if not match:
            continue
        op, startx, starty, endx, endy = match.groups()
    ...
    for instruction in instructions:
        ...
        for x in range(startx, endx + 1):
            for y in range(starty, endy + 1):
                if op == "turn on":
                    lights[x][y] += 1
                elif op == "turn off" and lights[x][y] > 0:
                    lights[x][y] -= 1
                elif op == "toggle":
                    lights[x][y] += 2

    return sum([l for r in lights for l in r])
