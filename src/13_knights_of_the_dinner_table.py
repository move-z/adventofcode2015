...
def first(*potentials):
    directory = {}

    for potential in potentials:
        match = re.match("(.+) would (gain|lose) (\d+) happiness units by sitting next to (.+).", potential)
        source, sign, value, dest = match.groups()
        if sign == 'gain':
            value = int(value)
        else:
            value = -int(value)
        if source not in directory:
            directory[source] = {}
        directory[source][dest] = value

    permutations = itertools.permutations(directory.keys())

    def calculate(permutation, i):
        p = permutation[i]
        pr = permutation[i - 1 if i > 0 else -1]
        nx = permutation[i + 1 if i < len(permutation) - 1 else 0]
        return directory[p][pr] + directory[p][nx]

    return max([sum([calculate(permutation, i) for i in range(len(permutation))]) for permutation in permutations])


def second(*potentials):
    directory = {}
